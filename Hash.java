...
public class Hash {
    ...
    public byte[] pack(byte[] text) throws Exception{
        return pack (text, nDatabytes, nCheckbytes, pattern, k);
    }
    public static byte[] pack (byte[] text, int n, int n2, byte b, int n3) throws Exception{
        int len = text.length, n4=n+n2+1,n5, n6=0;
        if(len%n==0){
            n5=len/n;
        }
        else{
    ...
    public byte[] unpack(byte[] ptext) throws Exception{
        return unpack (ptext, nDatabytes, nCheckbytes, pattern, k);
    }
